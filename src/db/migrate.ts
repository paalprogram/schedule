...
export function migrate(db: Database.Database) {
  ...
  try { db.exec(`ALTER TABLE student ADD COLUMN group_id INTEGER REFERENCES student_group(id)`); } catch { /* exists */ }
  try { db.exec(`ALTER TABLE student ADD COLUMN staffing_ratio INTEGER DEFAULT 1`); } catch { /* exists */ }
